...
const getSystemInstruction = (mode: AppMode, userReligion?: string) => {
...
export const chatWithGeminiStream = async (message: string, attachment: ChatAttachment | undefined, history: any[], mode: AppMode, userReligion?: string) => {
  const ai = getAIClient();
  const chat = ai.chats.create({
    model: 'gemini-3-flash-preview',
    config: {
      systemInstruction: getSystemInstruction(mode, userReligion),
    },
  });
