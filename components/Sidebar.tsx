...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const menuItems = [
    { id: ViewType.VISION, label: 'Vision Creator', icon: '🎨', badge: 'Soon' },
    { id: ViewType.PERSONALITY, label: 'Personality', icon: '🎭' },
    { id: ViewType.VERSION, label: 'Version History', icon: '📜' },
    { id: ViewType.DEBUG, label: 'Debug History', icon: '🛠️' },
  ...
  return (
    <>
      ...
      <div className={`fixed md:static inset-y-0 left-0 w-72 md:w-64 bg-[#171717] flex flex-col border-r border-white/5 transition-transform duration-300 z-50 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="mt-auto p-3 space-y-1 border-t border-white/5">
          <button 
          ...
          >
            Menu Tabs
            <svg className={`w-3 h-3 transition-transform ${isMenuVisible ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
          </button>
          
          <div className={`overflow-y-auto custom-scrollbar transition-all duration-300 ${isMenuVisible ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
            {menuItems.map(item => (
              <button
                key={item.id}
              ...
              >
                ...
                {item.badge && (
                  <span className="text-[8px] font-bold bg-indigo-500/20 text-indigo-400 px-1.5 py-0.5 rounded uppercase tracking-tighter">
                    {item.badge}
                  </span>
                )}
              </button>
